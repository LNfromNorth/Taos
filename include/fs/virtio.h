#ifndef __TAOS_VIRTIO_H__
#define __TAOS_VIRTIO_H__
...
static inline uint32_t in32(uint32_t *memory_address) {
    uint32_t result;
    asm volatile("lw %0, 0(%1)" // Load 32-bit value from memory into result
                 : "=r"(result) // Output constraint - tells the compiler to put
                                // the output in 'result'
                 : "r"(memory_address) // Input constraint - memory_address is
                                       // used as the memory location
    );
    return result;
}
...
static inline uint64_t io_to_virt(uint64_t pa) { return pa + IOMAP_OFFSET; }
