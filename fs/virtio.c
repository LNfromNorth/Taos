...
void virtio_blk_queue_init() {
    ...
    virtio_blk_regs->QueueAvailLow =
        0xffffffff & virt_to_phys((uint64_t)virtio_blk_ring.avail);
    virtio_blk_regs->QueueAvailHigh =
        virt_to_phys((uint64_t)virtio_blk_ring.avail) >> 32;
    virtio_blk_regs->QueueDescLow =
        0xffffffff & virt_to_phys((uint64_t)virtio_blk_ring.desc);
    virtio_blk_regs->QueueDescHigh =
        virt_to_phys((uint64_t)virtio_blk_ring.desc) >> 32;
    virtio_blk_regs->QueueUsedLow =
        0xffffffff & virt_to_phys((uint64_t)virtio_blk_ring.used);
    virtio_blk_regs->QueueUsedHigh =
        virt_to_phys((uint64_t)virtio_blk_ring.used) >> 32;
    memory_barrier();
...
void virtio_blk_config_init() {
    ...
    // uint64_t capacity =
    // ((uint64_t)config->capacity_hi << 32) | config->capacity_lo;
    return;
}
...
void virtio_blk_cmd(uint32_t type, uint32_t sector, void *buf) {
    ...
    virtio_blk_ring.avail
        ->ring[(virtio_blk_ring.avail->idx + 1) % VIRTIO_QUEUE_SIZE] = 0;
