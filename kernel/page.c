...
int page_map_make(pagetable_t pt, uint64_t va, uint64_t pa, uint64_t size,
                  int perm) {
...
void page_init() {
    ...
    // SBI
    ret += page_map_make(kpagetable, SBI_START, SBI_START,
                         (KERNEL_START - SBI_START), PTE_R | PTE_X);

    // kernel code
    ret += page_map_make(kpagetable, KERNEL_START, KERNEL_START,
                         ((uint64_t)_etext - KERNEL_START), PTE_R | PTE_X);

    // kernel data and other all
    ret += page_map_make(kpagetable, (uint64_t)_etext, (uint64_t)_etext,
                         (MEMORY_TOP - (uint64_t)_etext), PTE_R | PTE_W);
